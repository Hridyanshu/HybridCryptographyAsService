...
public class Decryption {
    public static void decrypt(String fileName, String key) throws IOException {
        ...
        while ((i=fr.read()) != -1) {
            if((char) i == ' ' || (char)i == '\n') {
                if(counter==1) {
                    temp2 = AES.decryption(temp, key);
                    fw.write(temp2);
                    fw.write((char)i);
                    counter++;
                }
                else if(counter==2) {
                    temp2 = RC4.decrypt(temp, key);
                    fw.write(temp2);
                    fw.write((char)i);
                    counter++;
                }
                else if(counter==3) {
                    temp2 = DES.decryption(temp, key);
                    fw.write(temp2);
                    fw.write((char)i);
                    counter=1;
                }
